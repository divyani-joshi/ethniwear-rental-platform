...
export default function ManageUsers({ setIsAuthenticated, adminName }) {
  ...
  const handleToggle = async (user) => {
    ...
    if (
      !window.confirm(
        `${ns === "Active" ? "Activate" : "Deactivate"} ${user.name}?`,
      )
    )
      return;
    setToggling(user._id);
    try {
      const r = await updateUserStatus({ user_id: user._id, status: ns });
      if (r.data.success) {
        toast.success(`User ${ns === "Active" ? "activated" : "deactivated"}!`);
        fetch();
      }
    } catch (err) {
    ...
    <AdminLayout setIsAuthenticated={setIsAuthenticated} adminName={adminName}>
      <div className="row mb-4">
        <div className="col-12">
          <h4 className="mb-1">Manage Customers</h4>
      ...
      {!loading && (
        <div className="row g-3 mb-4">
          {[
            ...
            {
              l: "Active",
              v: users.filter((u) => u.status === "Active").length,
            },
            {
              l: "Inactive",
              v: users.filter((u) => u.status === "Inactive").length,
            },
          ].map((s, i) => (
      ...
      <DataTable
        ...
        renderRow={(u, idx) => (
          <tr key={u._id}>
            ...
            <td>
              <span
                className={`badge badge-sm ${u.status === "Active" ? "badge-success" : "badge-danger"}`}
              >
                {u.status}
              </span>
            </td>
