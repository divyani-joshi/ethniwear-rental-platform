...
export default function ManageInventory({ setIsAuthenticated, adminName }) {
  ...
  const fetch = async () => {
    setLoading(true);
    try {
      const [invR, itemR] = await Promise.all([
        getInventory(),
        getAdminItems(),
      ]);
  ...
  const handleAdd = async (e) => {
    ...
    try {
      ...
      if (r.data.success) {
        ...
        setAddModal(false);
        setAddForm({ item_id: "", size_id: "", quantity: "", available: "" });
        fetch();
      }
    } catch (err) {
    ...
    <AdminLayout setIsAuthenticated={setIsAuthenticated} adminName={adminName}>
      <div className="row mb-4">
        <div className="col-12">
          ...
          <p className="text-sm text-muted mb-0">
            Track size availability and quantities for each clothing item.
          </p>
        </div>
      </div>
      ...
      <DataTable
        ...
        renderRow={(inv, idx) => {
          ...
          const pct = inv.quantity
            ? Math.round((inv.available / inv.quantity) * 100)
            : 0;
          return (
            <tr key={inv._id}>
              <td className="ps-4">
              ...
              <td>
                <p className="text-xs font-weight-bold mb-0">{inv.quantity}</p>
              </td>
              <td>
                <div>
                  <p className="text-xs font-weight-bold mb-1 text-success">
                    {inv.available}
                  </p>
              ...
              <td>
                <p className="text-xs font-weight-bold mb-0 text-warning">
                  {rented}
                </p>
              </td>
              <td>
                ...
                <div className="modal-body">
                  <div className="mb-3">
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setAddForm((f) => ({ ...f, item_id: e.target.value }))
                      }
                      required
                    >
                  ...
                  <div className="mb-3">
                    ...
                    <select
                      className="form-select"
                      value={addForm.size_id}
                      onChange={(e) =>
                        setAddForm((f) => ({ ...f, size_id: e.target.value }))
                      }
                      required
                    >
                    ...
                    {sizes.length === 0 && (
                      <small className="text-muted">
                        No sizes found. Ensure backend has sizes seeded.
                      </small>
                    )}
                  </div>
              ...
              <form onSubmit={handleEdit}>
                <div className="modal-body">
                  <div className="alert alert-light text-sm mb-3">
                    <strong>{editModal.item?.name}</strong> — Size:{" "}
                    <strong>{editModal.size?.size}</strong>
                  </div>
                  <div className="row g-3">
                    <div className="col-6">
                      <label className="form-label text-xs font-weight-bold text-secondary text-uppercase">
