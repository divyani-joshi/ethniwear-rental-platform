    ...
    <AdminLayout setIsAuthenticated={setIsAuthenticated} adminName={adminName}>
      <div className="row mb-4">
        <div className="col-12">
          <h4 className="mb-1">Payment Transactions</h4>
          <p className="text-sm text-muted mb-0">
            Monitor all rental payment records.
          </p>
        </div>
      </div>
      {!loading && (
          ...
          {[
            {
              ...
              c: "bg-gradient-success",
            },
            {
              l: "Successful",
              v: payments.filter((p) => p.status === "Success").length,
                ...
                <div className="card-body p-3">
                  <div className="row align-items-center">
                    ...
                    <div className="col-4 text-end">
                      <div
                        className={`icon icon-shape ${s.c} shadow text-center border-radius-md`}
                      >
                        <i className="fa fa-rupee-sign text-lg opacity-10" />
                      </div>
                    </div>
                  </div>
                </div>
      ...
      <DataTable
        ...
        data={payments}
        loading={loading}
        searchKeys={["user.name", "razorpay_payment_id", "status"]}
        ...
        renderRow={(p, idx) => (
          <tr key={p._id}>
            ...
            <td>
              <p className="text-xs mb-0 text-primary">₹{p.deposit_amount}</p>
            </td>
            <td>
              <p className="text-xs mb-0">₹{p.rent_amount}</p>
            </td>
            <td>
            ...
            <td>
              <span
                className={`badge badge-sm ${p.status === "Success" ? "badge-success" : "badge-danger"}`}
              >
                {p.status}
              </span>
            </td>
