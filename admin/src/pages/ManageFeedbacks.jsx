...
const Stars = ({ rating }) => (
  <div className="d-flex gap-1">
    {[1, 2, 3, 4, 5].map((s) => (
      <i
        key={s}
        className={`fa fa-star${s <= Math.round(rating) ? "" : "-o"}`}
        style={{ color: "#E3E231", fontSize: 12 }}
      />
    ))}
  </div>
);
    ...
    <AdminLayout setIsAuthenticated={setIsAuthenticated} adminName={adminName}>
      <div className="row mb-4">
        <div className="col-12">
          <h4 className="mb-1">Customer Feedbacks</h4>
          <p className="text-sm text-muted mb-0">
            Monitor all reviews from rental customers.
          </p>
        </div>
      </div>
      {!loading && (
        <div className="row g-3 mb-4">
          {[
            { l: "Total Reviews", v: feedbacks.length },
            { l: "Avg Rating", v: `${avg} / 5` },
            {
              l: "5 Star Reviews",
              v: feedbacks.filter((f) => f.rating === 5).length,
            },
          ].map((s, i) => (
      ...
      <DataTable
        title="All Feedbacks"
        columns={["Customer", "Item", "Rating", "Review", "Date"]}
        data={feedbacks}
        loading={loading}
        searchKeys={["user.name", "item.name", "feedback"]}
        ...
        renderRow={(f, idx) => (
          <tr key={f._id}>
            ...
            <td>
              <h6 className="mb-0 text-sm">{f.user?.name || "—"}</h6>
            ...
            <td>
              <p
                ...
                style={{ maxWidth: 220 }}
              >
                "{f.feedback?.slice(0, 80)}
                {f.feedback?.length > 80 ? "..." : ""}"
              </p>
            </td>
            <td>
              <p className="text-xs text-secondary mb-0">
                {f.datetime
                  ? new Date(f.datetime).toLocaleDateString("en-IN")
                  : "—"}
              </p>
            </td>
          </tr>
