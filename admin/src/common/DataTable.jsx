      ...
      <div className="card-header pb-0 d-flex justify-content-between align-items-center flex-wrap gap-3">
        ...
        <div className="d-flex gap-2 align-items-center flex-wrap">
          <div className="input-group" style={{ maxWidth: 260 }}>
            <span className="input-group-text"><i className="fa fa-search" style={{ fontSize: 12 }} /></span>
      ...
      <div className="card-body px-0 pt-0 pb-2">
        <div className="table-responsive p-0">
          <table className="table align-items-center mb-0">
            <thead>
              <tr>{["#", ...columns].map(c => <th key={c} className="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">{c}</th>)}</tr>
            </thead>
            <tbody>
              {loading ? (
                <tr><td colSpan={columns.length + 1} className="text-center py-5"><div className="spinner-border spinner-border-sm text-primary me-2" /><span className="text-secondary">Loading...</span></td></tr>
              ) : rows.length === 0 ? (
                <tr><td colSpan={columns.length + 1} className="text-center py-5"><i className="fa fa-search text-secondary mb-2 d-block" style={{ fontSize: 24 }} /><span className="text-secondary text-sm">{emptyMessage}</span></td></tr>
        ...
        {!loading && filtered.length > 0 && (
          <div className="d-flex justify-content-between align-items-center px-4 pt-3 flex-wrap gap-2">
            <small className="text-secondary">Showing {start + 1}–{Math.min(start + perPage, filtered.length)} of {filtered.length}</small>
